class AuctioneerTable {
  constructor(user) {
    this.user = user;
    this.id = user + "_view";
    this.eventClass = "org.example.empty.user";
  }
  loadData(data) {
    //console.log(data);
    ...
    for(var i = 0; i < data.length; i++) {
      ...
      var rowData = "";
      if(this.user === "index") {
        rowData = "<tr ><td width='10%'>" 
        + eventData["firstName"] + "</td><td width='10%'>" 
        + eventData["lastName"] + "</td><td width='10%'>" 
        + eventData["id"] + "</td><td width='15%'>" 
        + eventData["description"] + "</td><td width='10%'>" 
        + eventData["status"] + "</td><td width='10%'>" 
        + eventData["amount"] + "</td><td width='10%'>" 
        + eventData["buyerId"] + "</td></tr>";
      } else {
        rowData = "<tr ><td width='10%'>" 
        + eventData["type"] + "</td><td width='10%'>" 
        + eventData["ownerId"] + "</td><td width='10%'>" 
        + eventData["id"] + "</td><td width='15%'>" 
        + eventData["description"] + "</td><td width='10%'>" 
        + eventData["status"] + "</td><td width='10%'>" 
        + eventData["amount"] + "</td></tr>";
      }
  ...
  update(eventData) {
    if(eventData.$class === this.eventClass) {
      var rowData = "";
      if(this.user === "index") {
        rowData = "<tr class='anim highlight'><td width='10%'>" 
        + eventData["firstName"] + "</td><td width='10%'>" 
        + eventData["lastName"] + "</td><td width='10%'>" 
        + eventData["id"] + "</td><td width='15%'>" 
        + eventData["description"] + "</td><td width='10%'>" 
